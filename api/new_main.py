...
def run_training_job(job_id: str):
    """
    Exécute le pipeline de training en arrière-plan
    """
    try:
        ...
        # ✅ APPEL DE VOTRE FONCTION REFACTORISÉE
        results = run_training_pipeline()
        
        if results.get("status") == "success":
            training_jobs[job_id]["status"] = "completed"
            training_jobs[job_id]["completed_at"] = datetime.now().isoformat()
            training_jobs[job_id]["model_path"] = results.get("model_path")
            training_jobs[job_id]["metrics_path"] = results.get("metrics_path")
            training_jobs[job_id]["message"] = results.get("message")
            training_jobs[job_id]["results"] = results
            
            logger.info(f"✅ Job {job_id}: Training terminé avec succès")
        else:
            raise Exception(results.get("message", "Erreur inconnue"))
